...
import { requireAnalyticsToken } from '@/lib/db/analytics';

export async function GET(request: NextRequest) {
  const authHeader = request.headers.get('authorization');
  const queryToken = request.nextUrl.searchParams.get('token');

  if (!requireAnalyticsToken(authHeader, queryToken)) {
  ...
  return new NextResponse(html, {
    headers: {
      ...
      'Content-Security-Policy':
        "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'unsafe-inline'; img-src 'self' data:; connect-src 'self';",
    },
  });
}
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --primary: #5ba8e0; --primary-dim: rgba(91, 168, 224, 0.15);
    ...
    .range-btns button { padding: 0.4rem 1rem; border-radius: 0.5rem; border: 1px solid var(--border); background: var(--surface); color: var(--text-muted); cursor: pointer; font-size: 0.8rem; font-weight: 500; transition: all 0.15s; }
    .range-btns button:hover { border-color: var(--primary); color: var(--text); }
    .range-btns button.active { background: var(--primary-dim); border-color: var(--primary); color: var(--primary); }
    ...
    .panel { background: var(--surface); border: 1px solid var(--border); border-radius: 0.75rem; padding: 1.25rem; margin-bottom: 1rem; }
...
<body>
  <div class="container">
    ...
    <div id="content"><div class="loading">Loading analytics data&hellip;</div></div>
  </div>
  ...
  <script>
    (function() {
      ...
      function render(data) {
        ...
        const topPage = data.topPages && data.topPages[0];

        document.getElementById('content').innerHTML = [
          '<div class="cards">',
            card('Page Views', fmt(s.views || 0), ''),
            card('Unique Visitors', fmt(s.visitors || 0), ''),
      ...
      function renderCharts(data) {
        ...
        charts.views = new Chart(document.getElementById('chart-views'), {
          ...
          data: {
            ...
            datasets: [
              { label: 'Views', data: vot.map(d => d.views), borderColor: '#5ba8e0', backgroundColor: 'rgba(91,168,224,0.1)', fill: true, tension: 0.3, pointRadius: vot.length > 30 ? 0 : 3, pointHoverRadius: 5 },
              { label: 'Visitors', data: vot.map(d => d.visitors), borderColor: '#22d3ee', backgroundColor: 'rgba(34,211,238,0.08)', fill: true, tension: 0.3, pointRadius: vot.length > 30 ? 0 : 3, pointHoverRadius: 5 },
            ],
          },
          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top', labels: { usePointStyle: true, padding: 16 } } }, scales: { x: { grid: { color: gridColor }, ticks: { maxTicksLimit: 14 } }, y: { grid: { color: gridColor }, beginAtZero: true } } },
        });

        const COLORS = ['#5ba8e0', '#22d3ee', '#ec4899', '#34d399', '#fbbf24', '#f87171', '#818cf8', '#fb923c'];
        function doughnut(id, items, labelKey, valKey) {
          charts[id] = new Chart(document.getElementById(id), {
            type: 'doughnut',
            data: { labels: items.map(i => i[labelKey]), datasets: [{ data: items.map(i => i[valKey]), backgroundColor: COLORS.slice(0, items.length), borderWidth: 0 }] },
            options: { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'right', labels: { usePointStyle: true, padding: 12, font: { size: 11 } } } } },
          });
        }
